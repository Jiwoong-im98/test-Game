...
## 컨벤션
- 코드 스타일(들여쓰기, 네이밍 표기법 등)은 그 프로젝트가 쓰는 언어의 관용 스타일을 따르고, 확정되면 여기에 명시한다 (예: Python은 snake_case, JS/TS는 camelCase).
- 개발 방법론(TDD/SDD 등)을 쓰기로 했다면 여기에 명시한다.
- 에이전트의 코딩 습관(가정 명시, 최소 변경, 최소 코드)은 아래 Karpathy 가이드라인이 이미 담당하므로 중복 작성하지 않는다.
